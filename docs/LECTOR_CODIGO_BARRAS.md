...
- Para la mercadería que igual conviene etiquetar, el sistema le genera un
  código propio y lo imprime en planilla A4 con la **Epson EcoTank L1250**.
  Son dos pasos, una sola vez:

```
cd backend
venv\Scripts\activate
python manage.py asignar_codigos_barras --simular   # muestra qué haría
python manage.py asignar_codigos_barras             # asigna los códigos
```

  Y después, desde **Inventario → botón «Etiquetas»**, o por consola con
  `python manage.py imprimir_etiquetas`. El detalle del formato de planilla y
  de la impresora está en **`docs/perifericos.md`**.

  Los códigos que genera el sistema empiezan con **200**, que es el rango que
  GS1 reserva para uso interno de un comercio: así nunca chocan con el código
  real de un fabricante.

> ⚠️ Imprimir **siempre a escala 100%**. La opción "ajustar a la página" achica
> el código y el lector deja de leerlo, aunque la etiqueta se vea perfecta.
...
- La detección del lector está en
  `frontend/src/hooks/useLectorCodigoBarras.js`, con sus tests en
  `useLectorCodigoBarras.test.jsx`.
- Además de las pantallas del §3, el lector se usa en la **nota de pedido**
  (agrega el producto al pedido) y en **Inventario** (abre el panel de ajuste
  de la variante leída, que es el flujo de recepción de mercadería).
- Los códigos internos y la planilla de etiquetas están en
  `apps/productos/codigo_barras.py` y `apps/caja/impresora_a4.py`. Ver
  `docs/perifericos.md`.
