...
python manage.py test apps                    # 138 tests (facturación, productos, caja)
...
Test coverage is **partial**: 138 backend tests (`apps/facturacion/` full suite, plus `apps/productos/tests/`) and 22 frontend tests (`cd frontend && npm test`, vitest + jsdom — contextual help). Everything else has no automated tests — verify those changes manually against `docs/checklist_entrega.md`.
