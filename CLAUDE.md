...
python manage.py test apps                    # 165 tests (facturación, código de barras, escaneo, etiquetas)
...
Test coverage is **partial**: 165 backend tests (`apps/facturacion/` full suite, plus `apps/productos/tests/` for barcodes, `apps/inventario/tests/` for scan resolution and `apps/caja/tests/` for the A4 label sheets) and 44 frontend tests (`cd frontend && npm test`, vitest + jsdom — contextual help and the barcode-scanner hook). Everything else has no automated tests — verify those changes manually against `docs/checklist_entrega.md`.
