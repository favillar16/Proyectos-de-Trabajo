...
export function useLectorCodigoBarras({ onLectura, activo = true, largoMinimo = LARGO_MINIMO } = {}) {
  ...
  useEffect(() => {
    ...
    const alPresionar = (e) => {
      // Combinaciones de teclado del usuario (Ctrl+C, Alt+Tab...) no son
      // lecturas. Además invalidan lo acumulado: si quedara ahí, un Enter
      // posterior lo daría por leído y el sistema buscaría un código que
      // nunca se escaneó.
